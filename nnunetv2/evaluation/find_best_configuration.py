...
def generate_inference_command(dataset_name_or_id: Union[int, str], configuration_name: str,
    ...
    predict_command = ''
    trained_model_folder = get_output_folder(dataset_name_or_id, trainer_name, plans_identifier, configuration_name,
                                             fold=None)
...
def find_best_configuration(dataset_name_or_id,
    ...
    if allow_ensembling:
        for i in range(len(allowed_trained_models)):
            for j in range(i + 1, len(allowed_trained_models)):
                ...
                output_folder_1 = get_output_folder(dataset_name_or_id, m1['trainer'], m1['plans'], m1['configuration'],
                                                    fold=None)
                output_folder_2 = get_output_folder(dataset_name_or_id, m2['trainer'], m2['plans'], m2['configuration'],
                                                    fold=None)
                ...
                all_results[identifier] = \
                    {
                        ...
                        'result': load_summary_json(join(output_folder_ensemble, 'summary.json'))['foreground_mean'][
                            'Dice']
                    }

    # pick best and report inference command
    best_score = max([i['result'] for i in all_results.values()])
    best_keys = [k for k in all_results.keys() if
                 all_results[k]['result'] == best_score]  # may never happen but theoretically
    ...
    determine_postprocessing(all_results[best_key]['source'],
    ...
    return_dict = {
        ...
        'best_model_or_ensemble': {
            ...
            'result_on_crossval_post_pp':
                load_json(join(all_results[best_key]['source'], 'postprocessed', 'summary.json'))['foreground_mean'][
                    'Dice'],
    ...
    save_json(return_dict, join(nnUNet_results, dataset_name,
                                'inference_information.json'))  # save this so that we don't have to run this
    ...
    # print it
    print_inference_instructions(return_dict,
                                 instructions_file=join(nnUNet_results, dataset_name, 'inference_instructions.txt'))
    return return_dict
...
def print_inference_instructions(inference_info_dict: dict, instructions_file: str = None):
    ...
    for j, i in enumerate(inference_info_dict['best_model_or_ensemble']['selected_model_or_models']):
        ...
        _print_and_maybe_write_to_file(
            generate_inference_command(dataset_name_or_id, c, pl, tr, inference_info_dict['folds'],
